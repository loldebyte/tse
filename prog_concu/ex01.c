...
void shared_counter_drop(shared_counter_t * sc) {
    // TODO: Might need some change to work really in the way we expect ;)
    free(sc);
}
...
void * run(void * args) {
    ...
    printf("Bonjour depuis le thread n°%i.\n", data->thread_id);
    printf("Je m'appelle %ld\n", pthread_self());
    int not_finished = 1;
    while(not_finished) {
        if (data->shared_counter->current_owner == 0 || data->shared_counter->current_owner ==  (long int) pthread_self()) {
            data->shared_counter->current_owner = pthread_self();
            // Let's work thread! Increment counter 10000 times.
            for (int i = 0; i < 10000; i++) {
                shared_counter_increment(data->shared_counter);
            }
            data->shared_counter->current_owner = 0;
            not_finished = 0;
        }
        else {
